...
ZH = {
    ...
    "hint_arm": "设好后切回游戏窗口，按 Ctrl+Alt+T 即可瞬移（焦点不离开游戏，手柄不会失灵）",
    "fav_ask": "是否瞬移到「{name}」上空？",
    "ask_yes": "是",
    "ask_no": "否",
    ...
    "v_not_detected": "未检测到",
    "v_undetermined": "无法判断",
    "diag_hint_no_dll": "\n常见解决方式：把 SimConnect.dll 复制到“程序目录”中，然后重启本程序。",
    "diag_hint_copy": "\n提示：程序已找到 DLL，但库自己的包目录里还没有。本次连接会尝试自动复制；若仍失败，请把 DLL 手动复制到上面“SimConnect 包目录”中。",
    ...
    "arm_ok": "已设为快捷目标 {lat}, {lon}：切回游戏后按 Ctrl+Alt+T 瞬移",
    ...
    "teleport_ok": "已瞬移到 {lat}, {lon}（海拔 {alt} 米），已切回游戏窗口",
...
EN = {
    # ---- General / window ----
    "app_title": "MSFS-2024 Teleporter",
    "html_lang": "en",
    "h1": "MSFS-2024 Teleporter",
    ...
    "diag_hint_no_dll": "\nCommon fix: copy SimConnect.dll into the \"App directory\" above, then restart this tool.",
    "diag_hint_copy": "\nNote: the DLL was found, but it is missing from the library's own package dir. This run will try to copy it automatically; if it still fails, copy the DLL manually into the \"SimConnect package dir\" above.",
    ...
    "teleport_ok": "Teleported to {lat}, {lon} at {alt} m. Focus returned to the sim.",
    "teleport_fail": "Teleport failed: {err}",

    # ---- Footer credits (bottom of the side panel) ----
    "credit_design": "Designed by 肖阳",
    ...
    "js_prepare": "Preparing teleport: establishing connection…",
    "js_connect_failed": "Connection failed",
    "js_teleporting": "Teleporting…",
...
_JS_KEYS = [k for k in ZH if k.startswith("js_")] + [
    ...
    "diag_hint_no_dll", "diag_hint_copy",
...
def detect_lang():
    """语言判定顺序：环境变量 > EXE 文件名 > 系统 UI 语言 > 默认中文。

    现在只有单一 EXE：中文系统显示中文界面，其他语言系统显示英文界面。
    保留环境变量与 `-EN.exe` 文件名后缀作为手动覆盖手段。
    """
    import sys
